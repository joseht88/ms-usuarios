node {
   ...
   stage('Preparation') {
       git 'https://github.com/joseht88/ms-usuarios.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/ms-usuarios/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/ms-usuarios/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/ms-usuarios/runDeployment.sh'
       }catch (e){
